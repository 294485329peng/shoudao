...
<!--end  -->
 ...
 $(document).ready(function(){
	 $("#save").click(function(){
		 var ATTENTION_REPLY_ID = $('#ATTENTION_REPLY_ID').val();
		 if($("#switch_check").attr("checked") != "checked"){
			...
			if (!window.confirm('您确定要执行该操作吗？')) {
			 ...
			 if(ATTENTION_REPLY_ID == null || ATTENTION_REPLY_ID == ''){
					
					$.ajax({
						url: "${ctx}/attentionReply/save.do",
						type:"post",
						data: {"PUBLISH":"0"},
						dataType:"json",
					...
					$.ajax({
						url: "${ctx}/attentionReply/update.do",
						type:"post",
						data: {"PUBLISH":"0","ATTENTION_REPLY_ID":ATTENTION_REPLY_ID},
						dataType:"json",
						success: function(data) {
							BaseUtils.hideWaitMsg();
	 	 ...
	 	 var PIC_MEDIA_ID = $('#PIC_MEDIA_ID').val();
			...
			if(ATTENTION_REPLY_ID == null || ATTENTION_REPLY_ID == ''){
				...
				$.ajax({
					url: "${ctx}/attentionReply/update.do",
					type:"post",
					data: {"ATTENTION_REPLY_ID":ATTENTION_REPLY_ID,"IT_NAME":IT_NAME,"IT_GROUP_ID":IT_GROUP_ID,"IT_CREATE_TIME":IT_CREATE_TIME,"IT_PIC_URL":IT_PIC_URL,"PUBLISH":"1","TYPE":TYPE,"SAYTEXT":SAYTEXT,"PIC_ID":PIC_ID,"PIC_URL":PIC_URL,"PIC_MEDIA_ID":PIC_MEDIA_ID},
					dataType:"json",
					success: function(data) {
						BaseUtils.hideWaitMsg();
  ...
  */
  function selectIT(id) {
 		layer.open({
 			...
 			shadeClose : true,
 			shade : 0.8,
 			content : '${ctx}/attentionReply/selectIT.do'
 		});
 	}
  /**
   * 选择图文后返回数据调整
...
<div>
    ...
    <div class="mcontent" style="border-bottom: 0px;border-left: 0px;border-right: 0px;">
        <div class="alert">
            <p>注意：粉丝在关注您的公众号时，会自动发送您设置的文字，图片，图文给粉丝。</p>
            <p style="color: red; font-size: 16px !important">${exception.instruction } </p> 
        </div>
        <div style="${exception.flag ? 'display:none;' : ''}">
        <div class="switch_is">
        	开启关注回复：<input id="switch_check" class="js-switch" type="checkbox" ${record.PUBLISH eq '1' ? 'checked=""' : ''}  onchange="switchery();" />
		       ...
		       <span class="tab_navs_switch_wrp switch_next js_switch_next">
		           <span class="tab_navs_switch"></span>
		                   ...
		                   <div class="tab_navs_wrp">
		                   		<input id="ATTENTION_REPLY_ID" name="ATTENTION_REPLY_ID" type="hidden" value="${record.ATTENTION_REPLY_ID }" />
		                   		...
		                   		<input id="TYPE" type="hidden" name="TYPE" value="${record.TYPE eq null or record.TYPE eq '' ? '1' : record.TYPE}"/>
		                   ...
		                   <div id="editor_text" class="tab_content" style="${record.TYPE eq '1' ? 'display:block;' : 'display:none;'}">
		                       <div class="js_textArea inner no_extra">
		                           <div class="emotion_editor">
		                               ...
		                               <!-- <div style="overflow-y: attention; overflow-x: hidden;" class="edit_area js_editorArea1"
