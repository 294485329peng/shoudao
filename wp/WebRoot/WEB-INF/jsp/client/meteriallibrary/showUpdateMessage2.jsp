...
<div class="mcontent_padding">
	...
	<div id="admin_k" style="overflow:hidden;clear:both;">
		<div style="float:left;width:25%;">
			<div id=display0 class="pp" data-fileid="display0" PUBLISH="" data_id="0" ImageText_ID="" IT_GROUP_ID="">
					...
					<div class="">
						<div class="cover_appmsg_item" onmouseover="mouseover('header')" onmouseout="mouseout('header')">
							...
							<div class="appmsg_thumb_wrp">
								...
								<img id="cover_image" src="" style="width:100%;height:100%;"></img>								
							</div>
							<div class="fe" style="display: none">
								<a class="edit1" style="cursor:pointer;" onclick="goUpate('header','0','')"></a>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		var IT_ID = '${record.IT_ID}';
		var IT_GROUP_ID = '${record.IT_GROUP_ID}';
		var PUBLISH = '${record.PUBLISH}';
		BaseUtils.showWaitMsg();
		if(PUBLISH == '1'){
			$.ajax({
				url : "${ctx}/ml/getAdminIT.do",
				type : "post",
				data : {"IT_ID":IT_ID},
				dataType : "json",
				success : function(data) {
					if(data.flag){
						$('#display0').attr("ImageText_ID",IT_ID);
						$('#display0').attr("IT_GROUP_ID",data.obj.IT_GROUP_ID);
						$('#title_p').html("标题："+data.obj.TITLE);
						$('#cover').attr("style","display:none");
						$('#cover_image').attr("src", data.obj.PIC_URL);
				...
				success : function(data) {	
					if(data.flag){
						for(var i = 0; i < data.obj.length;i++){
							$("#display_parent").append("<div id=\"display"+data.obj[i].IT_ORDER+"\" class=\"appmsg_item\" data_id=\""+data.obj[i].IT_ORDER+"\" ImageText_ID=\""+data.obj[i].IT_ID+"\" onmouseover=\"mouseover(\'item\',\'show"+data.obj[i].IT_ORDER+"\')\" onmouseout=\"mouseout(\'item\',\'show"+data.obj[i].IT_ORDER+"\')\" >"
									+"<div id=\"show"+data.obj[i].IT_ORDER+"\" class=\"fd\" style=\"display:none\">"
										+"<a class=\"edit\" id=\"edit"+data.obj[i].IT_ORDER+"\" data_id=\""+data.obj[i].IT_ORDER+"\"  onclick=\"goUpate(\'display"+data.obj[i].IT_ORDER+"\',"+data.obj[i].IT_ORDER+",\'"+data.obj[i].IT_ID+"\')\" style=\"margin-top: 40px;cursor:pointer;\"></a>"
									+"</div>"
									+"<img id=\"imageId_"+data.obj[i].IT_ORDER+"\" class=\"appmsg_thumb\" alt=\"\" src=\""+data.obj[i].PIC_URL+"\">"
									+"<h4 class=\"appmsg_title\">"
										+"<a id=\"childtitle"+data.obj[i].IT_ORDER+"\" target=\"_blank\" href=\"#\">标题："+data.obj[i].TITLE+"</a>"
									+"</h4>"
								+"</div>");
						...
						if(length > 0){
							if(id <= data.obj[length-1].IT_ORDER){
								id = data.obj[length-1].IT_ORDER;
								id++;
							}
						}
					}
					...
					if(data.flag){
						for(var i = 0; i < data.obj.length;i++){
							$("#display_parent").append("<div id=\"display"+data.obj[i].IT_ORDER+"\" class=\"appmsg_item\" data_id=\""+data.obj[i].IT_ORDER+"\" ImageText_ID=\""+data.obj[i].IT_ID+"\" onmouseover=\"mouseover(\'item\',\'show"+data.obj[i].IT_ORDER+"\')\" onmouseout=\"mouseout(\'item\',\'show"+data.obj[i].IT_ORDER+"\')\" >"
									+"<div id=\"show"+data.obj[i].IT_ORDER+"\" class=\"fd\" style=\"display:none\">"
										+"<a class=\"edit\" id=\"edit"+data.obj[i].IT_ORDER+"\" data_id=\""+data.obj[i].IT_ORDER+"\"  onclick=\"goUpate(\'display"+data.obj[i].IT_ORDER+"\',"+data.obj[i].IT_ORDER+",\'"+data.obj[i].IT_ID+"\')\" style=\"margin-top: 40px;cursor:pointer;\"></a>"
										+"<a class=\"dete\" id=\"delete"+data.obj[i].IT_ORDER+"\" data_id=\""+data.obj[i].IT_ORDER+"\" style=\"cursor:pointer;\" onclick=\"goDelete(\'display"+data.obj[i].IT_ORDER+"\',"+data.obj[i].IT_ORDER+",\'"+data.obj[i].IT_ID+"\')\"></a>"
									+"</div>"
									+"<img id=\"imageId_"+data.obj[i].IT_ORDER+"\" class=\"appmsg_thumb\" alt=\"\" src=\""+data.obj[i].PIC_URL+"\">"
									+"<h4 class=\"appmsg_title\">"
										+"<a id=\"childtitle"+data.obj[i].IT_ORDER+"\" target=\"_blank\" href=\"#\">标题："+data.obj[i].TITLE+"</a>"
		  ...
		  if(save_type == 'not_save'){
		 	 ...
		 	 $("#display_parent").append("<div id=\"display"+id+"\" class=\"appmsg_item\" data_id=\""+id+"\" ImageText_ID=\"\" onmouseover=\"mouseover(\'item\',\'show"+id+"\')\" onmouseout=\"mouseout(\'item\',\'show"+id+"\')\" >"
											+"<div id=\"show"+id+"\" class=\"fd\" style=\"display:none\">"
												+"<a class=\"edit\" id=\"edit"+id+"\" data_id=\""+id+"\"  onclick=\"goUpate(\'display"+id+"\',"+id+",\'${news.id}\')\" style=\"margin-top: 40px;cursor:pointer;\"></a>"
												+"<a class=\"dete\" id=\"delete"+id+"\" data_id=\""+id+"\" style=\"cursor:pointer;\" onclick=\"goDelete(\'display"+id+"\',"+id+",\'${news.id}\')\"></a>"
											+"</div>"
		  ...
		  */
		  $("#publishIT").click(function() {
			  ...
			  /* if($("#display0").attr("PUBLISH") == "1"){
				  layer.msg('当前图文已经发布，无需重复发布，如果您更新并保存了图文可点继续！', {
					  	offset: '100px',
					    time: 0 //不自动关闭
					    ,btn: ['继续更新', '关闭']
					    ,yes: function(index){
	...
	function goUpate(symbol,data_id,newsId){
		//alert("111111");
		...
		$.ajax({
			...
			success:function(data){
					...
					$('#IT_TYPE').val(data.obj.IT_TYPE); 
				 	...
				 	$("#PIC_MEDIA_ID").val(data.obj.PIC_MEDIA_ID);
