...
$(document).ready(function(){
  $("#addNav").click(function(){
	  ...
	  window.location.href="${ctx}/webBgNav/forAddNav.do";
  });
});
function updateNav(NAV_ID){
	window.location.href="${ctx}/webBgNav/forUpdateNav.do?NAV_ID="+NAV_ID;
}
function deleteNav(id,pageNow){
...
<body>
    ...
    <div class="tools">
    	<form id="queryForm" action="${ctx}/webBgNav/navPic.do" method="post">
    ...
    <td><a href="#" onclick="updateNav('${r.NAV_ID}')">${r.TITLE }</a></td>
    <td>${r.CATEGORY }</td>
    <td>${r.RESPONSE eq '1' ? '商品详情' : '活动详情'}</td>
    <td><section class="model-2">
			  <div class="checkbox checkbox_normal">
			    <input id="PUBLISH_${status.index}" type="checkbox" ${r.PUBLISH eq 'on' ? 'checked=""' : ''} onchange="updateSwitchery('${r.NAV_ID}','PUBLISH_${status.index}');"/>
			    <label style="width: 54px; height: 33px;" class="checkbox-lebal-normal"></label>
			  </div></section></td>
    <td><a class="a_link"  style="cursor:pointer"  onclick="updateNav('${r.NAV_ID}')">编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="deleteNav('${r.NAV_ID}','${pageView.pageNow}')" class="a_link" style="cursor:pointer">删除</a> 
		</td>
    </tr>
    </c:forEach>
    <%-- <tr>
