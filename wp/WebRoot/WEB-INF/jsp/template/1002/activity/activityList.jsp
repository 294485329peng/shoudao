...
function goActivity(RESPONSE,ACTIVITY_ID,RESPONSE_ID){
	if("1" == RESPONSE){
		...
		window.location="${ctx}/TWeb/act/forActivity.htmls?store=${seTicket.gzhId}&ACTIVITY_ID="+ACTIVITY_ID;
	}
}
</script>
</head>
...
<body>
<div class="body">
<h3 class="fn-tit"  style="padding-bottom: 0px;background-color:#EEE9E9;padding-bottom: 5px;padding-top: 5px;"><a href="javascript:history.go(-1);" class="nback">返回</a><b class="tit">动态</b></h3>
  <div class="gb-contain" style="padding-top: 0px;padding-bottom: 0px;">
  	<form class="clearfix" action="${ctx}/TWeb/act/list.htmls?store=${seTicket.gzhId}" id="queryForm">
  	</form>
    <!-- <div class="gb-intro">
    ...
    <c:forEach var="r" items="${pageView.records}" varStatus="status">
      <li>
        <h3 class="tit" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;"><a onclick="goActivity('${r.RESPONSE}','${r.ACTIVITY_ID}','${r.RESPONSE_ID}');">${r.TITLE }</a></h3>
        <div class="gbl-intro clearfix" style=";"> <a onclick="goActivity('${r.RESPONSE}','${r.ACTIVITY_ID}','${r.RESPONSE_ID}');" class="gbl-img  floatl"><img style="width: 110px;height:110px;" class="img" src="${r.PIC_URL }" alt="图片"></a>
