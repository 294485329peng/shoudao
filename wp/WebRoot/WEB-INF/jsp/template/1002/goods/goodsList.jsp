...
$(document).ready(function (){
    					...
    					$("#ul_list").append(
    		    		        ...
    		    		        +"<div class=\"foodintro clearfix\"> <a href=\"\" class=\"foodimg floatl\"><img src=\""+jsonList[i].PIC_URL1+"\" style=\"max-height: 110px; min-height:80px;\"></a>"
    		    		          +"<div class=\"foodcon floatl\" style=\"float:right;\">"
    		    		            ...
    		    		            +"</div>"
    		    		            +"</div>"
    		    		            +"<em class=\"lt ltred\" style=\"font-size: 13px\">"
    		    		            +"</em> "
    		    		      +"</li>");
    					count++;
  ...
  $(".goodsDetail").click(function(){
	  ...
	  window.location.href="${ctx}/TWeb/goods/goodsDetail.htmls?store=${seTicket.gzhId}&GOODS_ID="+id ;
  });
});
function formSubmit(){
	$("#GOODS_NAME").val($("#goodsName").val());
	$("#form").submit();
}
</script>
</head>
...
<nav id="navmenu" class="navmenu">
	...
	<section class="navmenu-section">
		...
		<ul class="navmenu-section-list">
			...
			<li><a href="${ctx}/TWeb/goods/goodsList.htmls?store=${seTicket.gzhId}&CLASSIFY=${c.GOODS_CLASSIFY_ID}&CLASSIFY_NAME=${c.CLASSIFY_NAME }">${c.CLASSIFY_NAME }</a></li>
			</c:forEach>
		</ul>
	</section>
...
<div class="body" style=" background-color: #f4f4f4;">
  <div class="topsearch">
    <form class="clearfix" action="${ctx}/TWeb/goods/goodsList.htmls?store=${seTicket.gzhId}" id="form">
	  <a  title="搜索" class="floatl js-slideout-toggle" style="margin-top: 2px;"></a>
	  <input type="hidden" name="CLASSIFY" id="CLASSIFY" class="stxt"  value="${record.CLASSIFY }" >
      <input type="hidden" name="GOODS_NAME" id="GOODS_NAME" class="stxt"  value="${record.GOODS_NAME }" >
      <input type="text" name="goodsName" id="goodsName" class="stxt"  value="${record.GOODS_NAME }" >
      <input onclick="formSubmit();" type="button" class="sbtn icon-search" style="height: 23px;">
	</form>
  </div>
  <!--topsearch-->
  <div id="nav" class="container" >
    <ul class="assort clearfix nav-main">
      <li id="li-1" class="ast1 floatl ast-cur"><a >${empty record.CLASSIFY_NAME ? '全部' : record.CLASSIFY_NAME}<i class="icon-caret-down"></i></a></li>
    ...
    <c:forEach var="r" items="${pageView.records}" varStatus="status">
      <li id="li_${status.index}">
        ...
        <div class="foodintro clearfix"> <a class="foodimg floatl goodsDetail"><img src="${r.PIC_URL1 }" style="max-height: 110px; min-height:80px;"></a>
          <div class="foodcon floatl" style="float:right;">
            <p style="word-wrap: break-word;word-break:break-all;">单价：<b style="color:red;">￥${r.SHOW_PRICE }</b></p>
            <%-- <p class="fts orange">VIP价格：￥${r. }</p> --%>
            <p style="word-wrap: break-word;word-break:break-all;">积分：<b style="color:green;">${r.SHOW_POINT }分</b></p>
            <p style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">简介：${r.INTRODUCTION }</p>
        ...
        <em class="lt ltred" style="font-size: 13px"></em> 
      </li>
      </c:forEach>
      <%-- <li>
