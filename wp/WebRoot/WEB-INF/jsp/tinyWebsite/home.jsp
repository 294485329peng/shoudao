      ...
      <div id="banner_box" class="box_swipe">
        <ul>
         <c:forEach var="list" items="${record.mapNav}" varStatus="status">
        	  ...
        	  <li> <a onclick="goActivity('${list.RESPONSE}','${list.RESPONSE_ID}')"> <img src="${list.PIC_URL}" alt="" style="height: 250px;" /> </a>    
        	 </li>
        	 <%-- </c:forEach> --%>
        	 </c:forEach>
<%--           <li> <a onClick="return false;"> <img src="${ctx }/skin/tinyWebsite/images/goods1.jpg" alt="" style="height: 250px;" /> </a>    
  ...
  <em class="floatr tgtxt"><span style="text-align: left;font-size: 22px;word-break:break-all;height: 68px;">${listHac.TITLE }</span><a onclick="goActivity('${listHac.RESPONSE}','${listHac.RESPONSE_ID}');">查看详情</a></em>
  </div>
  </c:forEach>
...
function goActivity(RESPONSE,RESPONSE_ID){
	if("1" == RESPONSE){
		...
		window.location="${ctx}/TWeb/act/forActivity.htmls?ACTIVITY_ID="+RESPONSE_ID;
	}
}
</script>
<!-- <script type="text/javascript">
