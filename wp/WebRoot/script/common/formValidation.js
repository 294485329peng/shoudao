...
//正则表达式验证
function regexValidate(contentType,content,template,size){
	var formatStr = "";
	...
	}else if(contentType == "lengthCh"){
		//验证字符长度(一个字母一个字符，一个汉字两个字符)
		formatStr=/[^\u0000-\u00ff]/g;
		var lengthCh = content.replace(/[^\u0000-\u00ff]/g, "aa").length;
		if(lengthCh > size || lengthCh <= 0){
			return false;
		}else{
			return true;
		}
	...
	for(var i=0;i<jsonObj.length;i++){
		...
		if(str == "" || str == null){
			...
			if(contentType == "varchar"){
				...
				result = regexValidate(contentType,str,template,size);
			}
		}
