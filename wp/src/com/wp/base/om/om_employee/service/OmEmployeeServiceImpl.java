...
import com.wp.base.om.om_emp_org.service.IOmEmpOrgService;
...
/*		if(pId==null) {
	...
	public int register(Map record) {
    	...
    	String operatorId = "";
    	String now = DateUtils.getCurrentDate();
    	record.put("EMP_CODE", "registGzh");
    	record.put("EMP_STATUS", "0");
    	//操作员新增
    	if(!StringUtils.isEmpty(userId)) {
    		//操作员USER_ID重复检测
    		Map paramMap = new HashMap();
        	...
        	//record.put("REG_DATE", now);
        	record.put("CREATE_TIME", now);
        	//插入注册公众号管理组c35801ed-b99f-40a2-a517-2fe1d0ab11a6
            	Map empOrgRecord = new HashMap();
            	empOrgRecord.put("ORG_ID","c35801ed-b99f-40a2-a517-2fe1d0ab11a6");
            	empOrgRecord.put("EMP_ID",record.get("EMP_ID"));
            	empOrgRecord.put("IS_MAIN", "1");
            	dao.insert_("omEmpOrgSpace.insert", empOrgRecord);
        	//插入普通公众号组8433fee2-87e8-48ad-9d05-3a677d06db23
            	Map acOperatorRoleRecord = new HashMap();
            	acOperatorRoleRecord.put("OPERATOR_ID", operatorId);
            	acOperatorRoleRecord.put("ROLE_ID", "8433fee2-87e8-48ad-9d05-3a677d06db23");
            	dao.insert_("acOperatorRoleSpace.insert", acOperatorRoleRecord);
            //插入用户注册的其他信息比如邮箱等！
        	return dao.insert_("omEmployeeSpace.insert", record);
    	}
    		return 0;
