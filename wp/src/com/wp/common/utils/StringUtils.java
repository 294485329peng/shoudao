...
import com.wp.common.spring.ComponentFactory;
...
//		key = key.replace("-", "");
	...
	public static String uniqueKey36_() {// 36位长
		String key = UUID.randomUUID().toString();
		key = key.replace("-", "_");
		return key.toLowerCase();// .toUpperCase();
	}
	public static String uniqueKeyNumber(){
		Date dt= new Date();
    	Long time= dt.getTime();
    	int num = (int) Math.round(Math.random()*90000000+1);
    	String str = time.toString()+num;
		return str;
	}
	public static String uniqueKeyIpNumber(HttpServletRequest request){
    	...
    	String str = time.toString()+ip.replace(".", "")+num;
		return str;
	}
	/**
	 * 
